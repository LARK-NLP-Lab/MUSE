...
def parse(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        ...
        # else:
        #     results.append(None)
        if 'true' in ya.lower() or 'yes' in ya.lower():
            results.append(1)
        elif 'false' in ya.lower() or 'no' in ya.lower():
            results.append(0)
        else:
            results.append(None)
    df['output'] = results
...
def evaluate(df: pd.DataFrame, num_instances: int, dataset: str, setting: str):
    ...
    # y_true = pd.read_csv('../data/TQA/TQA_MUSE.csv')['y_true']
    ...
    # qid_order = (df.loc[df['round'] == 0].sort_values('round')['Qid'].tolist())
    
    bootstrap_df = pd.DataFrame(bootstrap_results)
    # print(len(qid_order))    
    # bootstrap_df.insert(0, 'Qid', qid_order)
    # TODO change to 0
    bootstrap_df.insert(0, "Instance ID", list(range(NUM_INSTANCES))) # was 1

    # setting = setting.strip('_textgen')
    # bootstrap_df.to_csv(f"../raw_probs/bootstrapped/{setting}_bs.csv", index=False)
